...
const ProductCard = memo(function ProductCard({ product }) {
  ...
  const handleWishlist = useCallback((e) => {
    ...
    // TODO: wire up wishlist API when available
  }, [])
  ...
  return (
    ...
    >
      ...
      <div className="relative aspect-square overflow-hidden bg-gray-50">
        ...
        <div className="absolute top-3 right-3 flex flex-col gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
          <button
            ...
            className={`w-8 h-8 bg-white rounded-lg shadow-sm flex items-center justify-center transition-colors ${
      ...
      <div className="p-4">
        ...
        <h3 className="font-semibold text-gray-900 mt-1 leading-tight line-clamp-2 group-hover:text-primary-600 transition-colors">
          {product.name}
        </h3>

        {/* Rating */}
        {product.reviewCount > 0 && (
          <div className="flex items-center gap-1.5 mt-2" aria-label={`Rated ${product.averageRating} out of 5, ${product.reviewCount} reviews`}>
            <div className="flex" aria-hidden="true">
              {Array.from({ length: 5 }).map((_, i) => (
                <Star
                  key={i}
                  size={12}
                  className={i < Math.floor(product.averageRating) ? 'text-yellow-400 fill-yellow-400' : 'text-gray-200 fill-gray-200'}
                />
              ))}
            </div>
        ...
        <div className="flex items-center gap-2 mt-3">
          ...
          {product.compareAtPrice && product.compareAtPrice > product.price && (
            <span className="text-sm text-gray-400 line-through" aria-label={`Original price ${formatCurrency(product.compareAtPrice)}`}>
        ...
        {/* Low stock warning */}
        {isLowStock && (
          <p className="text-xs text-orange-500 font-medium mt-1.5" role="status">
