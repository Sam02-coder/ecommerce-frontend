          ...
          <div>
            ...
            <div className="flex gap-3" role="list" aria-label="Social media links">
              {SOCIALS.map(({ Icon, href, label }) => (
                <a key={label} href={href} target="_blank" rel="noopener noreferrer" aria-label={label} role="listitem"
                  className="w-9 h-9 bg-gray-800 hover:bg-primary-500 rounded-xl flex items-center justify-center transition-colors focus:outline-none focus:ring-2 focus:ring-primary-400">
          ...
          <nav aria-label="Quick links">
            ...
            <ul className="space-y-2.5 text-sm">
              {QUICK_LINKS.map(({ label, to }) => (
                <li key={label}>
                  <Link to={to} className="hover:text-primary-400 transition-colors focus:outline-none focus:text-primary-400">{label}</Link>
                </li>
              ))}
            </ul>
          </nav>
            ...
            <address className="not-italic">
              <ul className="space-y-3 text-sm">
                ...
                <li className="flex items-center gap-3">
                  ...
                  <a href="mailto:support@shopzen.com" className="text-gray-400 hover:text-primary-400 transition-colors">support@shopzen.com</a>
                </li>
              </ul>
            </address>
            <div className="mt-5">
              ...
              <div className="flex gap-2" role="list" aria-label="Payment methods">
                {['Visa', 'MC', 'UPI', 'PayTM'].map((p) => (
                  <span key={p} className="px-2 py-1 bg-gray-800 rounded-lg text-xs font-medium text-gray-300" role="listitem">{p}</span>
                ))}
              </div>
            </div>
          </div>
